...
if (!class_exists('THWMSC_License_Manager')) :
	...
	class THWMSC_License_Manager
	{
		...
		public function output_license_page($params)
		{
			...
			$box_style = 'margin-top: 27px; padding: 40px 30px 10px 40px; background-color: #fff; border-radius: 4px; min-height: 600px; margin-left: 14px;';
			$box_left  = $box_style;
			$box_right = $box_style;

			if ($license_key) {
				$box_left  .= 'width: 44%; float:left;';
				$box_right .= 'width: 42%; float:left;';
			} else {
				$box_left  .= 'width: 92%;';
			}
?>
		...
		private function output_license_form($license_data)
		{
			...
			?>
			<h1><?php esc_html_e('Software License Key', 'woocommerce-multistep-checkout'); ?></h1>
			<?php echo $form_title_note; ?>
			<form method='post' action=''>
				<p style="position: relative;">
					<input type="text" <?php echo $license_field_attr ?> class="thwmscp_license_form">
					<span class="thwmscp-invalid-license-tooltip"><label class="thwmsc-license-error-message" style="color: white;">Invalid License</label></span>
					<?php echo wp_nonce_field('handle_license_form', 'nonce_license_form'); ?>
					<button type="submit" name="action" value="<?php echo $btn_action; ?>" class="thwmscp_admin_button thwmscp_save_btn" style="height: 52px !important; margin-left: 23px;"><?php echo $btn_label; ?></button>
				</p>
			</form>
		<?php
			...
			if (is_wp_error($request) || wp_remote_retrieve_response_code($request) != 200) {
				...
				if ($response_obj && (false === $response_obj->success)) {
					...
					add_action('admin_notices', function () use ($message) {
						echo '<div class="notice notice-error thwmscp_admin_notice thwmscp-plugin-license-error" style="background: #FFFFFF;border-radius: 5px;"><p>' . $message . '</p></div>';
					});
				} else {
					if ($action == 'activate') {
					...
					add_action('admin_notices', function () use ($message) {
						echo '<div class="notice notice-success" style="background: #FFFFFF;border-radius: 5px;"><p>' . $message . '</p></div>';
					});
				}
			}
		}
		...
		**/
			...
			*/
				...
				$notice = 'The license of <strong>%s</strong> is not activated. <a class="thwmscp-theme-color" href="%s">Click here</a> to activate the license.';
			}
		...
		?>
			<div class="error notice thwmscp_admin_notice <?php
