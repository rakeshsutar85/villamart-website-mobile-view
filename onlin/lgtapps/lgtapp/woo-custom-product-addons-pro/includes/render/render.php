...
//        add_action($this->action_hook_tag[0], array($this, 'render_form'), $this->action_hook_tag[1]);
    ...
    public function render_form($isRest = false)
    {
        if (!$this->product) {
            global $product;
...
//                                ) ? [] : ''; // No need to reset the default value, instead set the preSetValue as null
        ...
        return ['clones' => $clones, 'cart_key' => $cart_key];
    }
    ...
    public function get_pro_attr_list($pro = false)
    {
        ...
        foreach ($attributes as $key => $attribute) {
            ...
            if ($data['is_variation']) {
                //TODO when a custom variation set as variable,and then product switched back to single product, the custom attribute wont change its variation type,
