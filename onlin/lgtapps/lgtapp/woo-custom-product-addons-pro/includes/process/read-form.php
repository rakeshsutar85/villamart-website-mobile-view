...
//				$val  = str_replace( 'WCPAOTH ', '', $val );
            ...
            if ($field->type == 'productGroup') {
                ...
                $values_data[$j]['parentId'] = isset($item->parentId) ? $item->parentId : false;
                $values_data[$j]['image_id'] = isset($item->image_id) ? $item->image_id : false;
                $values_data[$j]['image']    = isset($item->image) ? $item->image : false;
                $values_data[$j]['quantity'] = 1;
            }
            if (isset($field->enable_quantity) && $field->enable_quantity) {
                $quantity                    = $this->fieldFromName($field->name, 'value', '_quantity');
                $values_data[$j]['quantity'] = is_array($quantity)?floatval($quantity[$j]):floatval($quantity);
            }
            if ($field->type == 'color-group') {
                $values_data[$j]['color'] = isset($item->color) ? $item->color : false;
            }
...
//        }
    ...
    public function readFieldFile($field)
    {
        ...
        if ($field->upload_type && $field->upload_type == 'basic') {
        ...
        } elseif ($field->upload_type && ($field->upload_type == 'ajax' || $field->upload_type == 'droppable')) {
            $allowedCount = 1;
            if (isset($field->multiple_upload) && $field->multiple_upload) {
                if (isset($field->max_file_count) && !empty($field->max_file_count)) {
                    $allowedCount = $field->max_file_count;
            ...
            $files  = json_decode(wp_unslash($_files));
