...
//                    $varValue = '('.$varValue.')';
    ...
    public function insert($title, $formJson, $formulaJson, $lang = false, $base_lang_id = false)
    {
        $my_post = array(
            'post_title'  => $title,
            'post_type'   => self::$CPT,
...
//                }
    ...
    public function save_form($post_id, $post_data)
    {
        ...
        if ($post_id === 0) {
            $new_post_id          = $this->insert($title, $fb_data_json, $fb_formula_json, $lang);
            ...
            $this->update($post_id, $title, $fb_data_json, $fb_formula_json, $lang);
        }
    ...
    public function update($post_id, $title, $formJson, $formulaJson, $lang = false)
    {
        update_post_meta($post_id, self::$META_KEY_2, $formJson);
        update_post_meta($post_id, self::$META_FORMULA_KEY, $formulaJson);
        ...
        if (isset($trans_data->values) && ( ! isset($trans_data->wpml_sync) || ! $trans_data->wpml_sync)) { //$trans_data->values
            foreach ($trans_data->values as $k => $v) {  // $trans_data->values as $k=>$v ( )
                foreach ($options as $ke => $va) { //   0=>label, 1=>value,2=>image
                    if (isset($v->{$va}) && ! isEmpty($v->{$va})) { // $trans_data->values items, $item->label, $item->value, so on
...
//        }
                ...
                if (is_uploaded_file($json_file["tmp_name"])) {
                    ...
                    if ( ! empty($posts)) {
                        foreach ($posts as $p) {
                                            ...
                                            if (isset($field->type) && ($field->type == "image-group" || $field->type == "select" || $field->type == "checkbox-group" || $field->type == "radio-group" || $field->type == "color-group" || $field->type == "productGroup")) {
                                                if (isset($field->values) && is_array($field->values)) {
                                                    foreach ($field->values as $index => $val) {
                                                        ...
                                                        if(isset($val->image)) {
                                                            $imageArray = array("id"  => $val->image_id, "url" => $val->image);

                                                            $imageField = isset($imageArray) ? $imageArray : ['id' => '', 'url'];
                                                        ...
                                                        if(isset($val->pimage) && !empty($val->pimage)) {
                                                            $pimageArray = array("id"  => $val->pimage_id, "url" => $val->pimage);

                                                            $pimageField = isset($pimageArray) ? $pimageArray : ['id' => '', 'url'=>''];
                            ...
                            $fields_data = json_decode(json_encode($fields_data), true);
    ...
    public function products_searching($search)
    {
        ...
        $searchOpts = get_posts($args);

        $searchOptions = [];
        if (is_array($searchOpts)) {
            foreach ($searchOpts as $v) {
                $searchOptions[] = [
                    'label' => get_the_title($v).' ('.$v->ID.')',
                    'value' => $v->ID
                ];
            }
        }
