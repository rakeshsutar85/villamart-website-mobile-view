...
class TM4B {
    ...
    // docs at: https://www.tm4b.com/en/sms-api/#toc_Example_2_Bulk_SMS
    ...
    public static function process_sms($api_key, $sender, $messages, $test_call) {
        ...
        foreach($chunked_messages as $message_batch) {
            ...
            curl_setopt($curl, CURLOPT_URL, 'https://api.tm4b.com/v1/sms');
            ...
            if ($is_success && $curl_error !== 0) {
                ...
                foreach($decoded_result['messages'] as $message) {
                    if (in_array($message['delivery_state'], ['REJECTED', 'FAILED', 'EXPIRED'])) {
                        $failed_sent[] = $message['destination_address'];
                    }
                }
            } else {
                $failed_sent += $mobiles;
