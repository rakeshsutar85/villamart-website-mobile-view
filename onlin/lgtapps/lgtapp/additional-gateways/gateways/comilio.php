...
class Comilio {
    ...
    // docs at: https://www.comilio.it/sms-gateway
    public static function sendSMS($gateway_fields, $mobile, $message, $test_call) {
        $authorization_token = $gateway_fields['authorization_token'];
        $sender = $gateway_fields['sender'];

        $last_sent_or_results = self::process_sms($authorization_token, $sender, [0 => [$mobile => $message]], $test_call);
    ...
    public static function sendBulkSMS($gateway_fields, $messages, $test_call) {
        ...
        return self::process_sms($authorization_token, $sender, $messages, $test_call);
    }

    public static function process_sms($authorization_token, $sender, $messages, $test_call) {
        $curl = curl_init();
        ...
        foreach($chunked_messages as $message_batch) {
            ...
            $data = array(
                'message_type' => 'Smart',
                'phone_numbers' => join(',', $mobiles),
                'text' => $fixed_message,
                'sender_string' => $sender,
            );

            curl_setopt($curl, CURLOPT_URL, 'https://api.comilio.it/rest/v1/message');
